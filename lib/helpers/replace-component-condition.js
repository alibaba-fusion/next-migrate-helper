...
module.exports = function replaceComponentCondition(oldName, oldProps, newName, warnInfo) {
    ...
    find(oldName, null, ast, j, {
      main: (path, localMainName, specifier, specifiers, oldMainIndex) => {
        ...
        if (oldMainName !== newManinName) {
          if (oldSubName) {
            ...
            const index = specifiers.findIndex(specifier => specifier.local.name === newManinName);
            if (index === -1) {
      ...
      element: (path, isOpen, element) => {
        ...
        if (oldProps) {
          ...
          const index = attributes ? attributes.findIndex(attribute => {
            const [name, value] = oldProps.split('=');
